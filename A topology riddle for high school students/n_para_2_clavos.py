...
def graph(s, n):
	...
	for i in range(1,n+1):
		plt.plot([3*i*pow(2,n)], [2*pow(2,n)], marker='o', markersize=3, color="red")
	...
	plt.axis([pow(2,n), pow(2,n)*(3*n+2), height - pow(2,n), 2*pow(2,n)*n+1])

	img=mpimg.imread('images/monalisa.jpg')
	#imgplot = plt.imshow(img)

	myaximage = plt.imshow(img, aspect='auto', extent=(px1,px2,height-pow(2,n),height), alpha=0.5, origin='upper', zorder=-1)
